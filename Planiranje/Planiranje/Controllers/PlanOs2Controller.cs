...
namespace Planiranje.Controllers
{
    public class PlanOs2Controller : Controller
    {
        ...
        public ActionResult Details(int id)
        {
            ...
            List<OS_Plan_2_aktivnost> osPlan1Aktivnosti = new List<OS_Plan_2_aktivnost>();
            if (podrucja.Count != 0)
            {
        ...
        [HttpPost]
        public ActionResult NovoPodrucje(PlanOs2View plan)
        {
            //test            
            ...
            int maxValue;
            List<OS_Plan_2_podrucje> trenutna_podrucja = new List<OS_Plan_2_podrucje>();
            trenutna_podrucja = baza.OsPlan2Podrucje.Where(w => w.Id_glavni_plan == _id).ToList();
            ...
            try
            {
                baza.OsPlan2Podrucje.Add(plan.Podrucje);
                baza.SaveChanges();
            }
            catch
            {
        ...
        public ActionResult PodrucjePomakGore(int id)
        {
            ...
            OS_Plan_2_podrucje plan = new OS_Plan_2_podrucje();
            plan = baza.OsPlan2Podrucje.Single(s => s.Id_plan == id);
            int id_glavni_plan = plan.Id_glavni_plan;
            int pozicija = plan.Red_br_podrucje;

            List<OS_Plan_2_podrucje> podrucja = new List<OS_Plan_2_podrucje>();
            podrucja = baza.OsPlan2Podrucje.Where(w => w.Id_glavni_plan == id_glavni_plan && w.Red_br_podrucje <= pozicija).ToList();
            podrucja = podrucja.OrderBy(o => o.Red_br_podrucje).ToList();
            ...
            OS_Plan_2_podrucje p = podrucja.ElementAt(podrucja.Count - 2);
            ...
            {
                var rezultat = db.OsPlan2Podrucje.SingleOrDefault(b => b.Id_plan == id_prethodni);
                if (rezultat != null)
                {
                    try
                    {
                        db.OsPlan2Podrucje.Attach(plan);
            ...
            using (var db = new BazaPodataka())
            {
                var rezultat2 = db.OsPlan2Podrucje.SingleOrDefault(d => d.Id_plan == id);
                if (rezultat2 != null)
                {
                    try
                    {
                        db.OsPlan2Podrucje.Attach(p);
        ...
        public ActionResult PodrucjePomakDolje(int id)
        {
            ...
            OS_Plan_2_podrucje plan = new OS_Plan_2_podrucje();
            plan = baza.OsPlan2Podrucje.SingleOrDefault(s => s.Id_plan == id);
            int id_glavni_plan = plan.Id_glavni_plan;
            int pozicija = plan.Red_br_podrucje;
            ...
            using (var db = new BazaPodataka())
            {
                var rezultat = db.OsPlan2Podrucje.SingleOrDefault(s => s.Id_plan == id);
                var rezultat2 = db.OsPlan2Podrucje.SingleOrDefault(d => d.Id_plan == id_nakon);
        ...
        public ActionResult UrediPodrucje(int id)
        {
            ...
            podrucje = baza.OsPlan2Podrucje.Single(s => s.Id_plan == id);
            ciljevi = ciljevi_db.ReadCiljevi();
        ...
        [HttpPost]
        public ActionResult UrediPodrucje(PlanOs2View plan)
        {
            int _id = 0;
            ...
            using (var db = new BazaPodataka())
            {
                ...
                var rezultat = db.OsPlan2Podrucje.SingleOrDefault(s => s.Id_plan == plan.Podrucje.Id_plan);
                if (rezultat != null)
                {
                    try
                    {
        ...
        [HttpPost]
        public ActionResult ObrisiPodrucje(PlanOs2View plan)
        {
            int _id = 0;

            using (var db = new BazaPodataka())
            {
                var rjesenje = db.OsPlan2Podrucje.SingleOrDefault(w => w.Id_plan == plan.Podrucje.Id_plan);
                ...
                if (rjesenje != null)
                {
                    ...
                    try
                    {
                        db.OsPlan2Podrucje.Remove(rjesenje);
                        db.SaveChanges();
                    }
                    catch
                    {
        ...
        public ActionResult NovaAktivnost(PlanOs2View plan)
        {
            int i = plan.Id;
            OS_Plan_2_aktivnost ak = new OS_Plan_2_aktivnost();
            ak = plan.Os_Plan_2_Aktivnost;
            ak.Id_podrucje = i;

            OS_Plan_2_podrucje p = new OS_Plan_2_podrucje();
            p = baza.OsPlan2Podrucje.Single(s => s.Id_plan == i);
        ...
        public ActionResult Details2(int id, int pozicija)
        {
            ...
            osPlan2Aktivnosti = osPlan2Aktivnosti.OrderBy(o => o.Red_broj_aktivnost).ToList();
            plan.OsPlan2Aktivnost = osPlan2Aktivnosti;
            ...
            OS_Plan_2_podrucje podr = new OS_Plan_2_podrucje();
            podr = podrucja.ElementAt(pozicija);
            osPlan2Aktivnosti = baza.OsPlan2Aktivnost.Where(w => w.Id_podrucje == podr.Id_plan).ToList();
            osPlan2Aktivnosti = osPlan2Aktivnosti.OrderBy(o => o.Red_broj_aktivnost).ToList();
            plan.OsPlan2Aktivnost = osPlan2Aktivnosti;
            TempData["prikaz"] = "1";
        ...
        public ActionResult AktivnostPomakGore(int id, int pozicija_podrucja)
        {
            ...
            OS_Plan_2_podrucje podrucje = new OS_Plan_2_podrucje();
            podrucje = baza.OsPlan2Podrucje.SingleOrDefault(s => s.Id_plan == aktivnost.Id_podrucje);

            int pozicija = aktivnost.Red_broj_aktivnost;

            List<OS_Plan_2_aktivnost> aktivnosti = new List<OS_Plan_2_aktivnost>();
            aktivnosti = baza.OsPlan2Aktivnost.Where(w => w.Id_podrucje == aktivnost.Id_podrucje && w.Red_broj_aktivnost <= pozicija).ToList();
        ...
        public ActionResult AktivnostPomakDolje(int id, int pozicija_podrucja)
        {
            ...
            List<OS_Plan_2_aktivnost> aktivnosti = new List<OS_Plan_2_aktivnost>();
            aktivnosti = baza.OsPlan2Aktivnost.Where(w => w.Id_podrucje == aktivnost.Id_podrucje && w.Red_broj_aktivnost >= aktivnost.Red_broj_aktivnost).ToList();
            aktivnosti = aktivnosti.OrderBy(o => o.Red_broj_aktivnost).ToList();
            if (aktivnosti.Count == 1)
            {
                return RedirectToAction("Details2", new { id = podrucje.Id_glavni_plan, pozicija = pozicija_podrucja });
